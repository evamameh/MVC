...
final class Application
{
    ...
    public static function bootstrap(string $basePath): self
    {
        ...
        $orm = ORM::fromConfig($dbConfig);
        $router = new Router();
        $request = Request::fromGlobals();
        $viewsPath = $basePath . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Views';
        $view = new Engine($viewsPath, $urlBase);

        $category = $orm->model(Category::class);
        $product = $orm->model(Product::class);
        $order = $orm->model(Order::class);
        $supplier = $orm->model(Supplier::class);
        $user = $orm->model(User::class);

        $container->instance(ORM::class, $orm);
        ...
        $container->instance(OrderController::class, new OrderController($view, $order, $product, $category));
